...
import { attributeLine } from './attribution.js';
...
export async function runDesignSystemCheck(options) {
  ...
  if (options.blame) {
    await addAttributions(violations, repoDir);
  }
  ...
  if (options.report) {
    ...
    await fs.writeFile(reportPath, formatMarkdownReport(violations, { includeAttribution: Boolean(options.blame) }), 'utf8');
  }
...
export function formatTextReport(violations) {
  ...
  for (const violation of violations) {
    ...
    if (violation.attribution) {
      lines.push(`  introduced by: ${violation.attribution.inventor} (${violation.attribution.relativeTime})`);
    }
...
export function formatMarkdownReport(violations, options = {}) {
  ...
  lines.push('');
  const hasAttribution = options.includeAttribution || violations.some((violation) => violation.attribution);
  lines.push(hasAttribution ? '| Location | Type | Asset | Introduced By | Diff |' : '| Location | Type | Asset | Diff |');
  ...
  for (const violation of violations) {
    ...
    if (hasAttribution) {
      lines.push(`| ${escapeMarkdown(`${violation.file}:${violation.line}`)} | ${escapeMarkdown(violation.type)} | ${escapeMarkdown(violation.assetName)} | ${formatAttributionCell(violation.attribution)} | ${diff} |`);
    } else {
...
async function addAttributions(violations, repoDir) {
  await Promise.all(violations.map(async (violation) => {
    const attribution = await attributeLine(violation.file, violation.line, { repoDir });
    if (attribution) {
...
function formatAttributionCell(attribution) {
  return attribution ? escapeMarkdown(`${attribution.inventor} (${attribution.relativeTime})`) : '';
}
