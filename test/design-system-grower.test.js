...
test('buildDiagnosis grades compact repos without competing families as A or B', () => {
  const diagnosis = buildDiagnosis({
    generatedAt: '2026-07-08T00:00:00.000Z',
    target: '/tmp/compact-ui',
    summary: {
      filesScanned: 4,
      competingFamilies: 0,
      roles: {
        Button: {
          variants: 1,
          competingFamilies: 0,
          topExample: 'inline-flex rounded px-3 py-2',
        },
        FormField: {
          variants: 1,
          competingFamilies: 0,
          topExample: 'block w-full rounded border px-3 py-2',
        },
      },
    },
    candidates: [],
  ...
  assert.match(diagnosis.grade, /^[AB]$/);
  assert.match(diagnosis.text, /Design System Diagnosis/);
  assert.match(diagnosis.text, /Repo: compact-ui/);
  assert.match(diagnosis.text, /UI Chaos Grade: [AB]/);
  assert.match(diagnosis.text, /Button: 1 variant, 0 competing families/);
  assert.match(diagnosis.text, /Generated by design-system-grower/);
});

test('buildDiagnosis grades repos with multiple competing families as D or worse', () => {
  const diagnosis = buildDiagnosis({
    generatedAt: '2026-07-08T00:00:00.000Z',
    target: '/tmp/drifty-ui',
    summary: {
      filesScanned: 6,
      competingFamilies: 2,
      roles: {
        Button: {
          variants: 5,
          competingFamilies: 2,
          topExample: 'inline-flex rounded-md bg-blue-600 text-white',
        },
        Alert: {
          variants: 3,
          competingFamilies: 0,
          topExample: 'rounded-md border bg-red-50 text-red-900',
        },
      },
    },
    candidates: [
      { id: 'candidate-001', actionType: 'canonicalize', score: 90 },
      { id: 'candidate-002', actionType: 'canonicalize', score: 70 },
    ],
    competingFamilies: [
      {
        id: 'drift-001',
        ...
        occurrences: 7,
        sides: [
          { side: 1, occurrences: 5, className: 'bg-emerald-50 border-emerald-200 text-emerald-900' },
          { side: 2, occurrences: 2, className: 'bg-green-50 border-green-200 text-green-900' },
        ],
      },
    ],
  });

  assert.match(diagnosis.grade, /^[DEF]$/);
  assert.match(diagnosis.text, /Worst competing family:/);
  assert.match(diagnosis.text, /side 1: 5 uses; bg-emerald-50/);
  assert.match(diagnosis.markdown, /```text\nDesign System Diagnosis/);
});
...
test('main writes a JSON catalog to the requested output path', async () => {
  ...
  const diagnosisMarkdown = await fs.readFile(path.join(fixtureDir, 'diagnosis.md'), 'utf8');
  ...
  assert.match(stdout, /Design System Diagnosis/);
  assert.match(stdout, /UI Chaos Grade: [A-F]/);
  ...
  assert.match(diagnosisMarkdown, /# Design System Diagnosis/);
  assert.match(diagnosisMarkdown, /```text\nDesign System Diagnosis/);
  assert.match(diagnosisMarkdown, /FormField: 1 variant, 0 competing families/);
...
test('diagnose command regenerates diagnosis markdown from existing artifacts', async () => {
  const fixtureDir = await makeFixtureRepo({
    'src/Form.tsx': `
      export function Form() {
        return <>
  ...
  await main([fixtureDir, '--out', path.join(designSystemDir, 'catalog.json')], {
    stdout: { write: () => {} },
  });
  await fs.rm(path.join(designSystemDir, 'diagnosis.md'));

  const exitCode = await main(['diagnose', designSystemDir], {
    stdout: { write: (message) => { stdout += message; } },
  });
  const diagnosisMarkdown = await fs.readFile(path.join(designSystemDir, 'diagnosis.md'), 'utf8');

  assert.equal(exitCode, 0);
  assert.match(stdout, /Design System Diagnosis/);
  assert.match(stdout, /Wrote diagnosis to /);
  assert.match(diagnosisMarkdown, /# Design System Diagnosis/);
  assert.match(diagnosisMarkdown, /```text\nDesign System Diagnosis/);
});
...
test('parseArgs rejects invalid options before running the scanner', () => {
  ...
  assert.deepEqual(parseArgs(['diagnose', 'design-system']), {
    command: 'diagnose',
    target: 'design-system',
  });
